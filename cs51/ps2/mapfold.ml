...
let filter_odd (nums : int list) : int list =
  List.filter (fun x -> x mod 2 = 1 || x mod 2 = ~-1) nums ;;
...
let super_sum (nlists : int list list) : int =
  List.fold_left (fun a x -> a + sum x) 0 nlists ;;
...
let filter_range (nums : int list) (range : int * int) : int list =
  let x, y = range in 
  List.filter (fun a -> a >= x && a <= y) nums ;;
...
let filter_by_year (slist : student list) (yr : year) : name list =
  List.map (fun s -> let n, y = s in n)
           (List.filter (fun s -> let n, y = s in y = yr) slist) ;;
